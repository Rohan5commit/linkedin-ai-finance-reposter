...
20. Added persistent anti-duplication state:
    ...
    - if all direct repost candidates are recently used, the run now falls back to article mode when fallback is enabled instead of silently succeeding with no post
    - when a history item is reused, it is moved to the newest history position so cooldown windows remain accurate
...
### K) Scheduler restoration (random twice weekly)

- User clarified they want random posting **twice per week**, not fixed weekdays.
- Restored daily cron trigger with random weekly gate:
  - cron: `0 9 * * *`
  - `RANDOMIZE_WEEKLY_RUN_DAYS=true`
- Why daily cron exists: GitHub cron cannot express “random weekdays,” so daily trigger + in-script gate is required for true random 2-day behavior.
...
7. Direct repost mode now enforces a one-week freshness window by default (`MAX_REPOST_AGE_DAYS=7`); if all candidates are stale, age-unknown, or recently used, article fallback should keep cadence when `DIRECT_REPOST_ARTICLE_FALLBACK=true`.
8. Article cooldown is a preference, not a hard stop: if it filters every current article candidate during fallback, the script relaxes it for that run and prefers the oldest recently used matching article so the scheduler does not complete successfully without posting or repeatedly pick the newest duplicate.
