...
class CadenceFallbackTests(unittest.TestCase):
    ...
    def test_append_recent_unique_value_moves_existing_value_to_end(self) -> None:
        result = main.append_recent_unique_value(["a", "b", "c"], "b", 3)

        self.assertEqual(result, ["a", "c", "b"])

    def test_article_flow_relaxation_prefers_oldest_recent_article(self) -> None:
        older_candidate = main.ArticleCandidate(
            title="Older AI finance story",
            url="https://example.com/older",
            source="Example News",
            summary_hint="Older but still acceptable fallback story.",
            published_at=datetime.now(timezone.utc),
            topic="ai-finance",
            score=10.0,
        )
        newer_candidate = main.ArticleCandidate(
            title="Newer AI finance story",
            url="https://example.com/newer",
            source="Example News",
            summary_hint="Newer repeated story.",
            published_at=datetime.now(timezone.utc),
            topic="ai-finance",
            score=500.0,
        )
        ...
        def build_post(candidate: main.ArticleCandidate, profile: main.LengthProfile) -> str:
            selected_titles.append(candidate.title)
            return "Generated post"

        with tempfile.TemporaryDirectory() as temp_dir, patch(
            "main.collect_candidates", return_value=[older_candidate, newer_candidate]
        ), patch("main.build_post", side_effect=build_post), patch(
            "main.choose_length_profile", return_value=main.SHORT_PROFILE
        ), patch("main.discover_preview_image_from_url", return_value=None):
            history_file = Path(temp_dir) / "article_history.json"
            main.save_article_history(
                str(history_file),
                [
                    main.normalize_article_url_for_history(older_candidate.url),
                    main.normalize_article_url_for_history(newer_candidate.url),
                ],
                10,
            )
            result = main.run_article_post_flow(
        ...
        self.assertEqual(result, 0)
        self.assertEqual(selected_titles, ["Older AI finance story"])
